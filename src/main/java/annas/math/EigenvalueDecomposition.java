 ...
 * ------------------------ */
   ...
   private void cdiv(double f, float xi, float yr, float yi) {
      float r,d;
      if (Math.abs(yr) > Math.abs(yi)) {
         r = yi/yr;
         d = yr + r*yi;
         cdivr = (float) ((f + r*xi)/d);
         cdivi = (float) ((xi - r*f)/d);
      } else {
         r = yr/yi;
         d = yi + r*yr;
         cdivr = (float) ((r*f + xi)/d);
         cdivi = (float) ((r*xi - f)/d);
      }
   }
   ...
   public static float hypot(float a, double f) {
      ...
      if (Math.abs(a) > Math.abs(f)) {
         ...
         r = (float) (Math.abs(f)*Math.sqrt(1+r*r));
 ...
 * ------------------------ */
   ...
   */
   ...
   public EigenvalueDecomposition (Matrix Arg) {
      float[][] A = Arg.getMatrix();
      n = Arg.getMatrix().length;
      V = new float[n][n];
      d = new float[n];
      e = new float[n];
   ...
   public Matrix getV () {
     float[][] ret = new float[this.n][this.n];
	   for(int i = 0; i<this.n;i++){
		   float div = this.V[0][i];
		   for(int j = 0; j<this.n;j++){
			   ret[j][i] = this.V[j][i]/div;

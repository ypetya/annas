...
public class TokenReader {
	...
	public void consumeSemiColon() {
		...
		while (this.position < this.stream.length
				&& (this.stream[this.position] == ';' || this.stream[this.position] == '\n')) {
			this.position++;
		}
	}
	...
	public String consumeToken() {
		...
		while (this.position < this.stream.length
				...
				&& !Character.isWhitespace(this.stream[this.position])) {
			this.position++;
		}
		return String.valueOf(Arrays.copyOfRange(this.stream, start,
				this.position));
	}
	...
	public boolean hasEndStmt() {
		this.consumeWhitespace();
		return !this.hasMoreTokens() || this.stream[this.position] == ';'
				|| this.stream[this.position] == '\n';
	}
	...
	public String nextToken() {
		...
		int start = this.position;
		int end = start;
		while (end < this.stream.length && this.stream[end] != ';'
				&& !Character.isWhitespace(this.stream[end])) {
			end++;
		}
